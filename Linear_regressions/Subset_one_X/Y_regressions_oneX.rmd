---
title: "Modeling expression as a function of Y chromosome copy number, samples with one X only"
...
metadata_lcl <- metadata[metadata$karyotype %in% c("X","XY","XYY","XYYYY") & metadata$cell_type == "LCL" & metadata$Technical_replicate == "N",]
rownames(metadata_lcl) <- metadata_lcl$sample

metadata_fib <- metadata[metadata$karyotype %in% c("X","XY","XYY","XYYYY") & metadata$cell_type == "Fib" & metadata$Technical_replicate == "N",]
...
#Load processed data
```{r}
lcl_counts <- read.delim(file=paste0(myPath,"Linear_regressions/Subset_one_X/lcl_txi_counts.txt"), check.names = FALSE)
fib_counts <- read.delim(file=paste0(myPath,"Linear_regressions/Subset_one_X/fib_txi_counts.txt"), check.names=FALSE)
...
dds_y_lcl <- DESeqDataSetFromMatrix(round(lcl_counts), colData = sample_table_lcl, design = ~ batch_libprep + y_count)
dds_y_lcl <- DESeq(dds_y_lcl)
save(dds_y_lcl, file="dds_y_lcl.rda")
# load(file="dds_y_lcl.rda")

lcl_normCounts <- counts(dds_y_lcl, normalized=TRUE)
# write.table(x=lcl_normCounts, file="lcl_normCounts.txt", sep="\t", quote=FALSE, col.names = TRUE, row.names = TRUE)

dds_y_fib <- DESeqDataSetFromMatrix(round(fib_counts), colData = sample_table_fib, design = ~ batch_libprep + y_count)
dds_y_fib <- DESeq(dds_y_fib)
save(dds_y_fib, file="dds_y_fib.rda")
# load(file="dds_y_fib.rda")

fib_normCounts <- counts(dds_y_fib, normalized=TRUE)
...
lcl_y_results <- allResults(mydds = dds_y_lcl, myVar = "y_count", myAnno = geneAnno, expGenes = lcl_expressed, myTitle = "lcl_y_count",myalpha = 0.05, pdfWidth = 3.5, pdfHeight = 3, myYlim = c(-2,1.5))
```
# fib Y Regression Results
```{r}
#Run results (Design ~ batch_libprep + y_count + x_count)
fib_y_results <- allResults(mydds = dds_y_fib, myVar = "y_count", myAnno = geneAnno, expGenes = fib_expressed, myTitle = "fib_y_count",myalpha = 0.05, pdfWidth = 3.5, pdfHeight = 3, myYlim = c(-2,1.5))
```

#Compare this regression with the regression including all samples
```{r}
#Bring in the autosomal genes from the regular Y regression in LCL and fib
reg_lcl_y_res <- read.delim(file=paste0(myPath,"Linear_regressions/LCLs/lcl_y_count_auto_expressed.txt"), stringsAsFactors = FALSE)
reg_lcl_y_res_sig <- reg_lcl_y_res[! is.na(reg_lcl_y_res$padj) & reg_lcl_y_res$padj < 0.05,]
reg_fib_y_res <- read.delim(file=paste0(myPath,"Linear_regressions/Fibroblasts/fib_y_count_auto_expressed.txt"), stringsAsFactors = FALSE)
reg_fib_y_res_sig <- reg_fib_y_res[! is.na(reg_fib_y_res$padj) & reg_fib_y_res$padj < 0.05,]

myData <- data.frame("x"= reg_lcl_y_res_sig[,"log2FoldChange"], "y" = lcl_y_results$res_anno_exp[reg_lcl_y_res_sig$Gene, "log2FoldChange"])
paper2_corrPlot_deming(myData = myData, myTitle = "lcl_Y_response_all_v_1X.pdf", myWidth = 2.35, myHeight = 2.35, myXlab = "Log2FC per Chr Y, all samples", myYlab = "Log2FC per Chr Y, 1 X", demingBehind = TRUE)

myData <- data.frame("x"= reg_fib_y_res_sig[,"log2FoldChange"], "y" = fib_y_results$res_anno_exp[reg_fib_y_res_sig$Gene, "log2FoldChange"])
paper2_corrPlot_deming(myData = myData, myTitle = "fib_Y_response_all_v_1X.pdf", myWidth = 2.35, myHeight = 2.35, myXlab = "Log2FC per Chr Y, all samples", myYlab = "Log2FC per Chr Y, 1 X", demingBehind = TRUE)
```
