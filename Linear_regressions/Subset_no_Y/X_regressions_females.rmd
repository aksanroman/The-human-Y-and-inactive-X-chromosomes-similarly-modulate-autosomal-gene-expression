---
title: "Modeling expression as a function of X chromosome copy number, females only"
output: html_notebook
---

#Setup
```{r}
myPath <- "/lab/solexa_page/adrianna/U01_RNAseq/2021_August_Analysis/Paper2_Github/"  #ADD PATH TO GITHUB FOLDER
#load DESeq2 library
...
metadata_lcl <- metadata[metadata$karyotype %in% c("X","XX","XXX","XXXX") & metadata$cell_type == "LCL" & metadata$Technical_replicate == "N",]
rownames(metadata_lcl) <- metadata_lcl$sample

metadata_fib <- metadata[metadata$karyotype %in% c("X","XX","XXX","XXXX") & metadata$cell_type == "Fib" & metadata$Technical_replicate == "N",]
...
# save(txi_lcl, file = paste0(myPath,"Linear_regressions/Subset_no_Y/txi_lcl.rda"))
load(file=paste0(myPath,"Linear_regressions/Subset_no_Y/txi_lcl.rda"))
...
# save(txi_fib, file=paste0(myPath,"Linear_regressions/Subset_no_Y/txi_fib.rda"))
load(file=paste0(myPath,"Linear_regressions_old/Subset_no_Y/txi_fib.rda"))
...
#Load processed data
```{r}
lcl_counts <- read.delim(file=paste0(myPath,"Linear_regressions/Subset_no_Y/lcl_txi_counts.txt"), check.names = FALSE)
fib_counts <- read.delim(file=paste0(myPath,"Linear_regressions/Subset_no_Y/fib_txi_counts.txt"), check.names=FALSE)
...
#Compare this regression with the regression using all samples
```{r}
...
paper2_corrPlot_deming(myData = myData, myTitle = "lcl_X_response_all_v_noY.pdf", myWidth = 2.35, myHeight = 2.35, myXlab = "Log2FC per Chr X, all samples", myYlab = "Log2FC per Chr Y, no Y", demingBehind = TRUE)

myData <- data.frame("x"= reg_fib_x_res_sig[,"log2FoldChange"], "y" = fib_x_results$res_anno_exp[reg_fib_x_res_sig$Gene, "log2FoldChange"])
paper2_corrPlot_deming(myData = myData, myTitle = "fib_X_response_all_v_noY.pdf", myWidth = 2.35, myHeight = 2.35, myXlab = "Log2FC per Chr X, all samples", myYlab = "Log2FC per Chr Y, no Y", demingBehind = TRUE)
```

#Compare male vs female only regressions for X response!
```{r}
#bring in male regression
lcl_x_count_1Y <- read.delim(file=paste0(myPath,"Linear_regressions/Subset_one_Y/lcl_x_count_auto_expressed.txt"), stringsAsFactors = FALSE)
row.names(lcl_x_count_1Y) <- lcl_x_count_1Y$Gene
fib_x_count_1Y <- read.delim(file=paste0(myPath,"Linear_regressions/Subset_one_Y/fib_x_count_auto_expressed.txt"), stringsAsFactors = FALSE)
row.names(fib_x_count_1Y) <- fib_x_count_1Y$Gene


#Still plotting normal sig genes!
myData <- data.frame("y"= lcl_x_count_1Y[reg_lcl_x_res_sig$Gene,"log2FoldChange"], 
                     "x" = lcl_x_results$res_anno_exp[reg_lcl_x_res_sig$Gene, "log2FoldChange"])

paper2_corrPlot_deming(myData = myData, myTitle = "lcl_X_response_male_v_female.pdf", myWidth = 2.35, myHeight = 2.35, myXlab = "Log2FC per Chr X, female samples (0Y)", myYlab = "Log2FC per Chr X, male samples (1Y)", demingBehind = TRUE)

myData <- data.frame("y"= fib_x_count_1Y[reg_fib_x_res_sig$Gene,"log2FoldChange"], 
                     "x" = fib_x_results$res_anno_exp[reg_fib_x_res_sig$Gene, "log2FoldChange"])


paper2_corrPlot_deming(myData = myData, myTitle = "fib_X_response_male_v_female.pdf", myWidth = 2.35, myHeight = 2.35, myXlab = "Log2FC per Chr X, female samples (0Y)", myYlab = "Log2FC per Chr X, male samples (1Y)", demingBehind = TRUE)
