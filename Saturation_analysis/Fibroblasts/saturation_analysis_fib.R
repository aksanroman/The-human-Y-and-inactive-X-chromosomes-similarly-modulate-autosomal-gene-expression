...
repeat {
  ...
  mySamples <-sample(1:dim(metadata_fib)[1], i, replace = FALSE)
  sample_table <- metadata_fib[mySamples,c("sample","x_count","y_count","batch_libprep")]
  ...
  if (is.fullrank(mod_npx)) {
    ...
    if(any(duplicated(sample_table[,2:4]))){
      ...
      x_results_auto <- na.omit(x_results[rownames(x_results) %in% fib_expressed$expAutoGenes,])
      x_results_auto_sig <- x_results_auto[x_results_auto$padj < 0.05,]
      ...
      y_results_auto <- na.omit(y_results[rownames(y_results) %in% fib_expressed$expAutoGenes,])
...
colnames(saturationResults) <- c("Sample_num", 
                                 "x_auto_sig","x_auto_up","x_auto_down",
                                 "y_auto_sig","y_auto_up","y_auto_down")
write.table(saturationResults, file=paste0(myPath,"Saturation_analysis/Fibroblasts/fib_saturationResults_100iterations_", as.character(i),".txt"), quote=FALSE, col.names = TRUE, row.names = FALSE, sep="\t")

list.save(resultSampleTables,file = paste0(myPath,"Saturation_analysis/Fibroblasts/fib_resultSampleTables_100iterations_",as.character(i),".rdata"))
